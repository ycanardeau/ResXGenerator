...
public sealed record GlobalOptions // this must be a record or implement IEquatable<T>
{
	...
	public GlobalOptions(AnalyzerConfigOptions options)
	{
		...
		PublicClass =
			options.TryGetValue("build_property.ResXGenerator_PublicClass", out var publicClassSwitch) &&
			publicClassSwitch is { Length: > 0 } &&
			publicClassSwitch.Equals("true", StringComparison.OrdinalIgnoreCase);

		NullForgivingOperators =
			options.TryGetValue("build_property.ResXGenerator_NullForgivingOperators", out var nullForgivingOperatorsSwitch) &&
		...
		if (options.TryGetValue("build_property.ResXGenerator_ClassNamePostfix", out var classNamePostfixSwitch))
		{
		...
		if (
			options.TryGetValue("build_property.ResXGenerator_InnerClassVisibility", out var innerClassVisibilitySwitch) &&
		...
		if (options.TryGetValue("build_property.ResXGenerator_InnerClassName", out var innerClassNameSwitch))
		{
			InnerClassName = innerClassNameSwitch;
		}

		InnerClassInstanceName = string.Empty;
		if (options.TryGetValue("build_property.ResXGenerator_InnerClassInstanceName", out var innerClassInstanceNameSwitch))
		{
		...
		if (
			options.TryGetValue("build_property.ResXGenerator_UseVocaDbResManager", out var genCodeSwitch) &&
